...
// Most of this commented out code is to do with having a language server like https://github.com/eclipse/eclipse.jdt.ls
// It might be helpful to anyone picking up this project in the future.
// And some of it is about checking if there is a valid Java version to run the server which is
// unnessecary if a jre is packed with the server. Or if the server is not written in Java.
// The code is from https://github.com/atom/ide-java

/*
const serverDownloadUrl = 'http://kodu.ut.ee/~karl97/beyond-language-server-0.0.1.zip'
const serverDownloadSize = 40881259
...
class JavaLanguageClient extends AutoLanguageClient {
  getGrammarScopes () { return [ 'source.beyond' ] }
  getLanguageName () { return 'Beyond' }
  getServerName () { return 'Beyond LS' }
  ...
  constructor () {
    ...
    //this.commands = {
    //  'java.ignoreIncompleteClasspath': () => { atom.config.set('ide-beyondql.errors.incompleteClasspathSeverity', 'ignore') },
    //  'java.ignoreIncompleteClasspath.help': () => { shell.openExternal('https://github.com/atom/ide-beyondql/wiki/Incomplete-Classpath-Warning') }
    //}
  }
...
/*
  startServerProcess (projectPath) {
    ...
    return this.checkJavaVersion(command)
      ...
      .then(dataDir => {
        ...
        const extraArgs = this.parseArgs(atom.config.get('ide-beyondql.virtualMachine.extraArgs'))
        args.push(...extraArgs)
        ...
        childProcess.on('close', exitCode => {
          if (!childProcess.killed) {
            atom.notifications.addError('ide-beyondql language server stopped unexpectedly.', {
              dismissable: true,
    ...
    return new Promise((resolve, reject) => {
      ...
      childProcess.on('error', err => {
        this.showJavaRequirements(
          'IDE-BeyondQL could not launch your Java runtime.',
          err.code == 'ENOENT'
      ...
      childProcess.on('close', exitCode => {
        ...
        if (exitCode === 0 && output.length > 2) {
          ...
          if (version == null) {
            this.showJavaRequirements(
              `IDE-Beyondql requires Java ${minJavaRuntime} but could not determine your Java version.`,
          ...
          if (version >= minJavaRuntime) {
            ...
            this.showJavaRequirements(
              `ide-beyondql requires Java ${minJavaRuntime} or later but found ${version}`,
          ...
          atom.notifications.addError('ide-beyondql encounted an error using the Java runtime.', {
            dismissable: true,
  ...
  showJavaRequirements (title, description) {
    atom.notifications.addError(title, {
      dismissable: true,
      buttons: [
        { text: 'Set Java Path', onDidClick: () => atom.workspace.open('atom://config/packages/ide-beyondql') },
  ...
  getJavaPath () {
    return (new Array(
      atom.config.get('ide-beyondql.javaHome'),
  ...
  installServer (serverHome) {
    const localFileName = path.join(serverHome, 'download.zip')
  ...
  actionableNotification (notification) {
    if (notification.message.startsWith('Classpath is incomplete.')) {
      switch(atom.config.get('ide-beyondql.errors.incompleteClasspathSeverity')) {
        case 'ignore': return
        case 'error': {
